...
function loadState() {
  const stored = localStorage.getItem('cbr_challenger_state');
  if (stored) {
    try {
      ...
      if (!appState.days || (appState.days.length !== 28 && appState.days.length !== 35)) {
        ...
        // Migration check for weaknesses
...
function saveState() {
  if (isViewingHistory) return; // Never save modifications while in read-only history view
